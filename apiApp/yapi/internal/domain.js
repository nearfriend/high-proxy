...
exports.fetchDomains = (req, res) => {
    if (!util.betterFileExists('./highProxy/config/ssl.json')) {
...
exports.addDomain = (req, res) => {
    ...
    let sslFileObj = JSON.parse(util.betterReadFile( './highProxy/config/ssl.json'))
    ...
    bashExec.on('exit', function (code) {
        ...
        if (code === 0) {
            ...
            return res.json({
                ...
                error: null,
                code: 0,
                message: `Successfully changed Domain ${domainName}, Please Restart highProxy to effect changes`,
                info: domainName,
            })
...
exports.deleteDomain = (req, res) => {
    ...
    if (domainValueList.indexOf(domainName) === -1) {
        ...
        return res.json({
            ...
            error: null,
            code: 0,
            message: `Successfully deleted Domain ${domainName}, Please Restart highProxy to effect changes`,
            info: domainName,
        })
    }
}
