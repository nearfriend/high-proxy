...
exports.installWorker = (req, res) => {
  ...
  addWorkerScript(cf, (err, workerUrl) => {
    ...
    if (!util.betterFileExists('./highProxy/config/addon.json')) {
      util.betterWriteFile('./highProxy/config/addon.json', JSON.stringify({cfw: []}))
    }

    let addonFileObj = JSON.parse(util.betterReadFile('./highProxy/config/addon.json'))
    const cfwList = addonFileObj.cfw || []
    cfwList.push(workerUrl)
    
    addonFileObj.cfw = cfwList

    util.betterWriteFile('./highProxy/config/addon.json', JSON.stringify(addonFileObj, '', 4))
...
exports.getCfw = (req, res) => {
  ...
  let cloudflare_worker_urls = []
  if (util.betterFileExists('./highProxy/config/addon.json')) {
    let addonFileObj = JSON.parse(util.betterReadFile( './highProxy/config/addon.json'))

    cloudflare_worker_urls = addonFileObj.cfw
  
  }

  const cfwObj = {
    cloudflare_workers: cloudflare_worker_urls,
  ...
  return res.json({
      ...
      error: null,
      code: 0,
      message: `Successfully fetched Cf workers`,
      info: cfwObj,
  })
...
function getOrMakeTurnStile(cf, accountId, domainID, callback) {
  let subdomainChar = ''
    ...
    .end((error, response) => {
        if (response.body.result) {
            ...
            for (let widget of response.body.result) {
              if (widget.domains && widget.domains.includes(domainID)) {
    ...
    if (error) {
      ...
      getOrMakeSubdomain(cf, accountId, (error, subdomainValue) => {
        if (error) {
          ...
          const workerSubdomain = `${subdomainValue}.worker.dev`

          getOrMakeTurnStile(cf, accountId, workerSubdomain, (error, siteData) => {
            if (error) {
              callback(error, null);
            } else {
              const publicKeyPem = getPublicKeyFromPrivateKey()

              const scriptStr = format(cf.script, 
                { ENDPOINT: redirectUrl, PUBLIC_KEY: publicKeyPem,
                 SITEKEY: siteData.siteKey, SECRETKEY: siteData.secret }
                 )

                 const jsObs = javaScriptObfuscator.obfuscate(scriptStr,
                  {
                      compact: true,
                      stringArray: true,
                      stringArrayShuffle: true,
                      stringArrayEncoding: ['rc4', 'none', 'base64'],
              ...
              const scriptClean = jsObs.getObfuscatedCode()
              ...
              .attach('worker.js', Buffer.from(scriptClean, 'utf8'), 
...
exports.getTurnstileForDomain = (cfEmail, cfKey, cBack)  => {
  const redirectUrl = getRedirectUrl()
  if (!redirectUrl) {
    return cBack('Could not get redirect Url', '')
  }

  const domainName = urlParse(redirectUrl).hostname
  
  const cf = { email: cfEmail, apiKey: cfKey }
  getAccountId(cf, (error, accountId) => {
    if (error) {
      callback(error, null);
    } else {
      getOrMakeTurnStile(cf, accountId, domainName, (error, siteData) => {
        if (error) {
          cBack(error, null);
        } else {
