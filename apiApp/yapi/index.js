...
const guardLib = require('./internal/guard')
...
controllerRouter.get('/handler/redirect/disable', redirectLib.disableRedirect);
...
controllerRouter.get('/handler/cfw',  cfWLib.getCfw)
...
controllerRouter.post('/handler/antibot/switch', [
    check('antibot').default(true),
    check('antibotType').default(''),
    check('antibotInfo').default(''),
    check('hcaptchaData').default(''),
    check('cfData').default('')
],
guardLib.changeGuard);
...
controllerRouter.post('/handler/proxy', [
    check('proxyUrl').exists(),
],
execLib.setProxyUrl)
